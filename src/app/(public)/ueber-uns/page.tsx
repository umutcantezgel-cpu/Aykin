...
export default function AboutPage() {
  ...
  return (
    <main className="min-h-screen bg-[#FAF8F5] overflow-x-hidden w-full max-w-full">
      <SubpageHero 
        label="ÜBER UNS"
      ...
      <section className="py-24 px-6 md:px-12 w-full max-w-full overflow-hidden flex flex-col items-center justify-center">
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="w-full md:w-2/3 flex flex-col gap-8 text-center md:text-left">
              <p className="text-2xl md:text-4xl text-[#2D2D2D] leading-relaxed font-light">
      ...
      <section ref={containerRef} className="py-24 relative bg-white overflow-hidden w-full max-w-full">
      ...
      <section className="py-24 px-6 md:px-12 bg-[#1A1A1A] text-white overflow-hidden w-full max-w-full">
        <div className="max-w-7xl mx-auto">
...
function TimelineItem({ item, isEven, idx }: any) {
  return (
    <motion.div 
      ...
      className={`flex flex-col md:flex-row items-center w-full max-w-full mb-24 md:mb-32 ${isEven ? 'md:flex-row-reverse' : ''}`}
    >
      {/* Spacer for symmetry (only visible on md+) */}
      <div className="hidden md:block flex-1 w-full" />
      
      {/* Node */}
      <div className="shrink-0 w-20 h-20 rounded-full bg-[#1A1A1A] border-4 border-white shadow-2xl flex items-center justify-center text-white my-8 md:my-0 z-20 relative group mx-4 md:mx-8">
      ...
      <div className="flex-1 w-full max-w-full">
        <div className={`bg-[#FAF8F5] p-8 md:p-16 rounded-[3rem] border border-[#E8D5C4] hover:border-[#C41E3A]/50 hover:shadow-2xl transition-all duration-700 relative overflow-hidden group text-center ${isEven ? 'md:text-right' : 'md:text-left'}`}>
          ...
          <div className={`text-[#C41E3A] font-calistoga text-8xl md:text-9xl opacity-[0.03] absolute -top-10 pointer-events-none transform group-hover:scale-110 transition-transform duration-700 ${isEven ? '-left-10' : '-right-10'}`}>
            {item.year}
          </div>
          ...
          <div className="relative z-10">
            ...
            <p className="text-[#2D2D2D] text-lg md:text-xl leading-loose">{item.desc}</p>
          </div>
        </div>
      </div>
    </motion.div>
...
function ValueCard({ icon, title, desc, delay = 0 }: any) {
  return (
    <motion.div 
      ...
      className="bg-[#2a2a2a] p-10 md:p-12 rounded-[3rem] border border-white/10 hover:border-[#C41E3A]/50 transition-colors duration-500 group flex flex-col items-center md:items-start text-center md:text-left"
    >
      ...
      <h3 className="font-calistoga text-3xl mb-4 leading-tight">{title}</h3>
